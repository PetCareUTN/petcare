...
import {
  ...
  UpdateDisponibilidadVeterinariaRequest,
...
export class DisponibilidadesVeterinariasService {
  ...
  private readonly baseUrl = `${environment.apiUrl}/disponibilidades-veterinarias`;

  getMine(): Observable<DisponibilidadVeterinariaResponse[]> {
    return this.http
      .get<DisponibilidadVeterinariaResponse[]>(`${this.baseUrl}/mia`, {
  ...
  replaceMine(
    data: UpdateDisponibilidadVeterinariaRequest,
  ): Observable<DisponibilidadVeterinariaResponse[]> {
    return this.http
      .put<DisponibilidadVeterinariaResponse[]>(`${this.baseUrl}/mia`, data, {
        headers: this.authHeaders(),
      })
      .pipe(catchError((error: HttpErrorResponse) => this.mapError(error)));
  }
