...
import { UpdateProfileRequest, UserProfile } from '../models/profile';
...
export class ProfileService {
  ...
  getMe(): Observable<UserProfile> {
    return this.http
      .get<UserProfile>(`${this.apiUrl}/auth/me`, { headers: this.authHeaders() })
      .pipe(catchError((error: HttpErrorResponse) => this.mapError(error)));
  }

  update(data: UpdateProfileRequest): Observable<UserProfile> {
    return this.http
      .patch<UserProfile>(`${this.apiUrl}/users/me`, data, { headers: this.authHeaders() })
      .pipe(catchError((error: HttpErrorResponse) => this.mapError(error)));
  }
